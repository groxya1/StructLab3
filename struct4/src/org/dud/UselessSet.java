...
public class UselessSet{//целый бесполезный класс
    ...
    public UselessSet Union(UselessSet set1, UselessSet set2) {
        if (set1.Count() == 0) return set2;
        else if (set2.Count() == 0) return set1;
        UselessSet result = new UselessSet();
    ...
    public UselessSet GetOnlyCompositeNumbers()
    {
        if (this.Count()==0) return this;
        UselessSet compositeNums = new UselessSet();
        for (int value:this.values)
            if (!isPrime(value)) compositeNums.Add(value);
        return compositeNums;
    }
    public UselessSet GetOnlyPrimeNumbers()
    {
        if (this.Count()==0) return this;
        UselessSet primeNums = new UselessSet();
        for (int value:this.values)
